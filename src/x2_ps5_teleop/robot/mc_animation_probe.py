...
from .mc_animation import ARM_NAMES, compile_animation, hold_animation, positions
...
def main():
    ...
    try:
        ...
        measured = probe.frame()
        frames = single_joint_frames(measured, args.joint, args.delta)
        animation = compile_animation(frames)
        ...
        report.update(joint=args.joint, delta=args.delta, csv_sha256=animation.sha256,
                      source_rate_hz=20, csv_tick_ms=2, csv_rows=animation.rows)
        if not args.execute:
        ...
        if attempted and not hold_sent:
            try:
                probe.wait(0.05)
                hold = hold_animation(probe.frame())
